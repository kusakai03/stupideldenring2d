...
public class PlayerAttribute : MonoBehaviour
{
    ...
    public WeaponPhysic currentWeapon {get; private set;}
    ...
    private void ChargeAttack(object sender, EventArgs e)
    {
        ...
        currentWeapon?.SetDamage(finalATK * (dmgMultiplier + 0.5f + playerMoving.GetChargeTime()), chargeAtkStaminaCost);
    ...
    private void NormalAttack(object sender, EventArgs e)
    {
        ...
        currentWeapon?.SetDamage(finalATK * dmgMultiplier, normalAtkStaminaCost);
        if (effect.effects.Any(a => a.effectType == "Bleed")){
            TakeDamage(GetNumberByPercent(finalHP, 5), 0, "", transform);
        }
    }
    ...
    public void UpdateWeapon(WeaponData weapon, WeaponPhysic newWeapon){
        if (weapon){
    ...
    public void TakeDamage(float damage, float poise, string damageType, Transform objecthit){ //Damage type: Physical, Fire, Ice, Lightning, Magic
        if (playerMoving.state != PlayerMoving.playerState.Evade && playerMoving.CanHitPlayer()){
            ...
            if (playerMoving.state == PlayerMoving.playerState.Block){ //Nếu đang đỡ đòn thì giảm thêm sát thương
                switch (damageType){
                    case "Physical":
            ...
            Instantiate(floatingDamage, transform.position, Quaternion.identity).GetComponent<TextMeshPro>().text = lastDamageTaken.ToString();
        }
    }
