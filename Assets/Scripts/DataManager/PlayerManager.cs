...
public class PlayerManager : MonoBehaviour
{
    ...
    public event EventHandler onHoldingArrow;
    public event EventHandler onShootingArrow;
    ...
    private IEnumerator LoadSceneAndRespawn()
    {
        ...
        PlayerAttribute p = Instantiate(playerObject, savePos, Quaternion.identity).GetComponent<PlayerAttribute>();
        currentPlayer = p.gameObject;
        p.lv.SetLevel(str,dex,vit,intg);
        List<EquipmentData> elis = new(){armor,glove,helmet,leg,ring1,ring2,ring3,ring4,weapon1,weapon2};
        List<string> keys = new(){"armor", "glove", "helmet", "leg", "ring1", "ring2", "ring3", "ring4", "weapon1", "weapon2"};
        for(int i = 0; i < keys.Count; i++){
            p.Equipment(elis[i], keys[i]);
        }
        SaveData();
    ...
    public void HoldingArrow(){
        onHoldingArrow?.Invoke(this, EventArgs.Empty);
    }
    public void ShootingArrow(){
        onShootingArrow?.Invoke(this, EventArgs.Empty);
    }
